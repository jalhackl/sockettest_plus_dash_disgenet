import eventlet
eventlet.monkey_patch(builtins=True, subprocess=True)
#from https://community.plotly.com/t/triggering-callback-from-within-python/23321/5 (narragansett)
import sockettest
...
tab1 = html.Div(children=[
    ...
    html.P("Exclude gene-disease-edges with a score below (this slider is shared via socket-io sockettest custom-element):"),
    dcc.Slider(id="scorethresholdslider", min=0, max=1, step = 0.05, value=0.6, 
               marks={0: '0', 1: '1'}),

    #sockettest dummy
    html.Div(id='dummy_thresholdslider'),
...
#sockettest callback for threshold-slider
@dash_app.callback(Output('dummy_thresholdslider', 'children'), [Input('scorethresholdslider', 'value')])
def func(value):
	change('scorethresholdslider', {'value': value})
	print('scorethresholdslider', value)
	return None

def change(id, val):
	print("slider value changed for all clients")
	socketio.emit('call', {'id': id, 'val': val})
